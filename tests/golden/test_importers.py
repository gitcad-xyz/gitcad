...
def test_fcstd_import_returns_the_step_migration_path() -> None:
    """FreeCAD .FCStd geometry is OCCT-format .brp, which gitcad no longer
    reads (ADR-0020) — the import must return the STEP migration path, not a
    parse error or a silent None."""
    from gitcad.mcp.server import REGISTRY

    result = REGISTRY["model_import"](path="legacy.FCStd")
    assert result["ok"] is False
    assert "STEP" in result["error"]["message"]
    assert "FreeCAD" in result["error"]["message"]
