...
occt = pytest.importorskip("gitcad.kernel.occt")
...
def _build(*features):
    ...
    for f in features:
        doc.add(f)
    return doc.build(KERNEL).final(doc)


def test_ruled_loft_is_a_frustum():
    shape = _build(Feature(op="loft", params={
...
def test_loft_needs_two_sections():
    from gitcad.errors import KernelError

    with pytest.raises(KernelError, match="at least 2"):
        KERNEL.loft([(_square(10), 0.0)])


def test_straight_sweep_is_a_prism():
    shape = _build(Feature(op="sweep", params={
        "profile": _square(2), "path": [[0, 0, 0], [0, 0, 20]]}))
    assert KERNEL.measure(shape)["volume"] == pytest.approx(80.0, rel=1e-6)


def test_sweep_refuses_offset_path():
    from gitcad.errors import KernelError

    with pytest.raises(KernelError, match="start at"):
        KERNEL.sweep(_square(2), [(5.0, 0.0, 0.0), (5.0, 0.0, 20.0)])


def test_mirror_fuse_doubles_a_half_body():
    box = Feature(op="box", params={"dx": 10, "dy": 10, "dz": 5})
    doc = Document()
    bid = doc.add(box)
    ...
    assert KERNEL.measure(shape)["volume"] == pytest.approx(1000.0, rel=1e-6)
    lo, _hi = KERNEL.bbox(shape)
    assert lo[2] == pytest.approx(-5.0, abs=1e-6)


def test_countersink_hole_removes_cone_ring():
    box = Feature(op="box", params={"dx": 20, "dy": 20, "dz": 10})
    doc = Document()
    bid = doc.add(box)
...
def test_mass_props_of_a_box():
    shape = KERNEL.box(10, 20, 30)
    p = KERNEL.mass_props(shape)
