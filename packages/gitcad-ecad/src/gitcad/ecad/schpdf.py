...
def sheet_to_pdf(sch: Schematic) -> bytes:
    ...
    def stroke(width_mm: float) -> None:
        s.append(f"{float(width_mm * MM):.3f} w")

    def line(x1, y1, x2, y2, width=0.25) -> None:
        stroke(width)
        s.append(f"{float(X(x1)):.3f} {float(Y(y1)):.3f} m {float(X(x2)):.3f} {float(Y(y2)):.3f} l S")

    def polyline(pts, width=0.25, close=False) -> None:
        stroke(width)
        (px, py), *rest = pts
        s.append(f"{float(X(px)):.3f} {float(Y(py)):.3f} m")
        for qx, qy in rest:
            s.append(f"{float(X(qx)):.3f} {float(Y(qy)):.3f} l")
    ...
    def circle(cx, cy, r, width=0.25, fill=False) -> None:
        ...
        rp = r * MM
        rkp = rk * MM
        s.append(f"{float(px + rp):.3f} {float(py):.3f} m")
        s.append(f"{float(px + rp):.3f} {float(py + rkp):.3f} {float(px + rkp):.3f} {float(py + rp):.3f} {float(px):.3f} {float(py + rp):.3f} c")
        s.append(f"{float(px - rkp):.3f} {float(py + rp):.3f} {float(px - rp):.3f} {float(py + rkp):.3f} {float(px - rp):.3f} {float(py):.3f} c")
        s.append(f"{float(px - rp):.3f} {float(py - rkp):.3f} {float(px - rkp):.3f} {float(py - rp):.3f} {float(px):.3f} {float(py - rp):.3f} c")
        s.append(f"{float(px + rkp):.3f} {float(py - rp):.3f} {float(px + rp):.3f} {float(py - rkp):.3f} {float(px + rp):.3f} {float(py):.3f} c")
    ...
    def text(x, y, size_mm, value, *, anchor_end=False) -> None:
        ...
        s.append(f"BT /F1 {float(size_mm * MM):.2f} Tf {float(tx):.3f} {float(Y(y)):.3f} Td ({_esc(value)}) Tj ET")
    ...
    for sym in gfx.get("symbols", {}).values():
        for shp in sym["shapes"]:
            ...
            if shp["kind"] == "rect":
                ...
                stroke(0.25)
                s.append(f"{float(X(ax1)):.3f} {float(Y(ay1)):.3f} m "
                         f"{float(X(c1x)):.3f} {float(Y(c1y)):.3f} {float(X(c2x)):.3f} {float(Y(c2y)):.3f} "
                         f"{float(X(ax2)):.3f} {float(Y(ay2)):.3f} c S")
